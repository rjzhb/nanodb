...
public abstract class Expression implements Cloneable {
     ...
     * and finds all symbols that appear within the expression.  It is used by
     * the {@link #getAllSymbols(Collection)} method.  It has two major modes
     * of operation.  First, it can be used to identify whether or not an
     * expression has symbols (using the {@link #SymbolFinder()} constructor),
     * and/or it can be used to identify the symbols themselves (using the
     * {@link #SymbolFinder(Collection)} constructor).
     * </p>
     * <p>
     * This symbol-finder ignores two important situations:  First, it ignores
     * wildcard expressions, even {@code tablename.*} wildcards.  Second, it
     * ignores subqueries.
     * </p>
     */
    private static class SymbolFinder implements ExpressionProcessor {
        ...
        /**
         * If not {@code null}, this is the collection of symbols that have
         * been found in the expression.
         */
        private Collection<ColumnName> symbols;

        /**
         * Construct a symbol-finder that can identify whether an expression
         * has symbols or not, as well as optionally collecting the symbols
        ...
        /**
         * Construct a symbol-finder that only identifies whether an
         * expression has symbols or not, but does not collect the symbols
         * themselves.
         */
        ...
        public void enter(Expression e) {
            if (e instanceof ColumnValue) {
                ...
                if (symbols != null) {
                    ...
                    ColumnName cn = cv.getColumnName();
     ...
     * This method may only be used if the expression doesn't contain any
     * symbolic values.  If the expression does contain symbols then an
