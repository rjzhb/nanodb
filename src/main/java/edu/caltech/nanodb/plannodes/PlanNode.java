...
/**
 * The base class of all query execution plan nodes.  All plan nodes have
 * similar implementation characteristics provided by this class.  They may
 * have zero, one or two child plan nodes.
 *
 * <h2>Marking</h2>
 *
 * Plan nodes may or may not support marking positions within the
 * tuple-stream produced by the node.  Some algorithms require marking
 * positions within a node's tuple stream, so that processing can return to
 * that point later on.
 * <p>
 * Implementations are encouraged to only support marking if it is easy to
 * support in the implementation.  The {@link MaterializeNode} can be used to
 * add marking to any tuple stream.
 * <p>
 * If a plan node supports marking, its behavior must be as follows:
 *
 * <ul>
 *     <li>Marking should be inexpensive, and there should be no penalty for
 *     not resetting to a given mark.</li>
 *
 *     <li>When {@link #resetToLastMark} is called, the next call
 *     to {@link #getNextTuple} should return the first tuple that would be
 *     returned after the most recent call to {@link #markCurrentPosition}.</li>
 *
 *     <li>It must be possible to mark before the first tuple.  Resetting to
 *     this mark should cause the next call to {@link #getNextTuple} to again
 *     return the first tuple.  (Note that this is similar to the case when
 *     {@link #initialize} is called on a plan node, but the difference here
 *     is that marking/resetting uses cached results, and {@link #initialize}
 *     causes the results to be regenerated from scratch.)</li>
 *
 *     <li>It must be possible to mark after the last tuple.  Resetting to
 *     this mark should cause the next call to {@link #getNextTuple} to return
 *     {@code null}.</li>
 * </ul>
 *
 */
public abstract class PlanNode implements Cloneable {
     ...
     *
     * @throws IllegalArgumentException if <tt>leftChild</tt> is
     *         <tt>null</tt>
     */
    ...
    public void markCurrentPosition() {
        throw new UnsupportedOperationException("Marking is unsupported");
    }
